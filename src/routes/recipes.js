...
function SectionLink(props) {
  return (
    <HashLink smooth to={`/recipes/#${props.recipeSection}`}>
...
function SectionMenu() {
  ...
  return (
    <nav className="mx-4 md:mx-8 lg:mx-16 py-2 flex space-x-8 border-t border-b justify-center overflow-x-auto">
      {sections}
    </nav>
  );
}
    ...
    <li className="md:w-1/2 lg:w-1/3 shrink p-3">
      <Link to={`/recipes/${props.recipeId}`} key={props.recipe.name}>
        <img
          ...
          alt="recipe"
        ></img>
        <p className="caption border-b border-l border-r p-2">
          {props.recipe.name}
        </p>
      </Link>
    </li>
...
function RecipeSections() {
  const listItems = allRecipes_keys.map((recipeSection) => (
    <div
      key={recipeSection}
      id={recipeSection}
      className="mx-4 md:mx-8 lg:mx-16 mt-10"
    >
