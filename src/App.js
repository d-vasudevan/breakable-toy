  ...
  return (
    <li className="pt-4 pb-4">
      <a href={props.link} target="_blank" rel="noreferrer">
        <Icons componentName={props.icon} />
      </a>
    </li>
  );
}
...
function SMLinkNav(props) {
  ...
  if (props.menunav === 'true') {
    //checking if the sm links are in the main nav or in the slide in nav
...
function SlideInMenuLinks(props) {
  ...
  return (
    <li>
      <Link to={link} onClick={slideInMenu}>
        {props.link}
      </Link>
    </li>
  );
...
function SlideInMenuNav(props) {
  return (
    <nav
      id={props.id}
      className="fixed top-0 left-0 h-screen w-full md:w-96 lg:w-60 lg:ml-16 bg-white border-black md:border-r hidden bg-bg flex flex-col space-y-8 justify-center items-center"
    >
...
function MainNav() {
  return (
    <nav className="fixed top-0 left-5 md:top-3 md:left-8 lg:top-0 lg:left-0 lg:relative flex flex-col lg:w-20 justify-between items-center lg:h-screen pt-5 pb-10 lg:border-r lg:border-black lg:bg-bg">
      <button onClick={slideInMenu} className="z-10">
